...
import { getBookingConfirmationEmailTemplate } from '@/lib/email-templates/booking-confirmation'
...
export async function POST(request: NextRequest) {
  try {
    const body: SendBookingConfirmationRequest = await request.json()
    const {
      organization_id,
      to,
      customer_name,
    ...
    const formattedPaid = `${amount_paid.toFixed(2)} ${currencySymbol}`

    // Get email template
    const { subject, body: emailBody } = getBookingConfirmationEmailTemplate({
