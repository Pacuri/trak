...
const LANGUAGE_OUTPUT_INSTRUCTIONS = {
  rs: {
    instruction: 'ALWAYS output all user-facing text (names, descriptions, notes, warnings) in Serbian (Latin script, ekavica dialect). TRANSLATE from source language if needed. Use Serbian terminology: cena, noćenje, polupansion, deca, smeštaj, prevoz, doplata, upozorenje, besplatno. Example: "Djeca" → "Deca", "cijena" → "cena", "dijete" → "dete", "predviđeni" → "predviđeni".',
    example_terms: { price: 'cena', child: 'dete', accommodation: 'smeštaj', supplement: 'doplata', warning: 'upozorenje', free: 'besplatno' }
  },
  ba: {
 ...
 */
export function getDocumentParsePrompt(languageRegion: LanguageRegion = 'rs'): string {
 ...
 */
export function getSystemPrompt(languageRegion: LanguageRegion = 'rs'): string {
