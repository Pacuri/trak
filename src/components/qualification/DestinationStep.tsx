...
interface DestinationStepProps {
  value: { country: string; city: string | null }
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-500">Izaberite destinaciju ili pretražite</p>
      </div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
        {filteredDestinations.map((destination) => (
          <button
            ...
            type="button"
            onClick={() => onChange(destination.country)}
            className={`
      ...
      {searchQuery && !filteredDestinations.some((d) => d.country.toLowerCase() === searchQuery.toLowerCase()) && (
        <button
          type="button"
        ...
        >
          <span className="text-gray-500">+ Dodaj &quot;{searchQuery}&quot; kao destinaciju</span>
        </button>
      )}
      ...
      {value.country && (
        <div className="mt-4">
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Konkretan grad? (opciono)
          </label>
          <input
            type="text"
            placeholder={`npr. grad u ${value.country}`}
            value={value.city || ''}
